...
export const OPENAI_SCHEMA = {
  ...
  properties: {
    ...
    servings: { type: 'integer', description: '레시피 기준 인원 수' },
    ingredients: {
      type: 'array',
      description: '재료 목록',
      items: {
        ...
        properties: {
          ...
          amount: { type: ['number', 'null'], description: '재료 양 (숫자. 없으면 생략)' },
          unit: { type: ['string', 'null'], description: '재료 단위' },
        },
    ...
    steps: {
      ...
      items: {
        type: 'object',
        properties: {
          step_order: { type: 'integer', description: '조리 순서 (1, 2, 3...)' },
          instruction: {
            type: 'string',
            description:
              '조리 지시사항. 재료의 무게/부피 계량 수치(예: 300g, 1큰술)는 명시하지 마라. 단, 조리 시간(N분)이나 온도(N도) 관련 텍스트는 무조건 보존할 것.',
          ...
          step_ingredients: {
            ...
            items: {
              ...
              properties: {
                name: { type: 'string', description: '재료 이름' },
                amount: { type: ['number', 'null'], description: '재료 양' },
                unit: { type: ['string', 'null'], description: '재료 단위' },
              },
              required: ['name', 'amount', 'unit'],
              additionalProperties: false,
        ...
        required: ['step_order', 'instruction', 'timer_seconds', 'step_ingredients'],
        additionalProperties: false,
      },
    },
  },
